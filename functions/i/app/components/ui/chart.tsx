...
type ChartLegendContentProps = ToggleGroupProps &
      ...
      {payload.map((item: LegendPayload) => {
        ...
        return (
          <Toggle
            ...
            id={key}
            className={twMerge(
              "*:data-[slot=icon]:-mx-0.5 flex items-center gap-2 *:data-[slot=icon]:size-2.5 *:data-[slot=icon]:shrink-0 *:data-[slot=icon]:text-muted-fg",
            )}
            intent="plain"
