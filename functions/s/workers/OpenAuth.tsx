...
import { Email, UserSubject } from "~/lib/Domain";
import { IdentityMgr } from "~/lib/IdentityMgr";
import * as Q from "~/lib/Queue";

type A = Schema.Schema.Type<typeof UserSubject>;
type I = Schema.Schema.Encoded<typeof UserSubject>;

export type Subjects = {
  readonly user: v1.StandardSchema<
    Schema.Schema.Encoded<typeof UserSubject>, // Encoded type (I) is input
    Schema.Schema.Type<typeof UserSubject> // Decoded type (A) is output
  >;
};
      ...
      Effect.gen(function* () {
        ...
        if (env.ENVIRONMENT === "local") {
          yield* Effect.tryPromise(() =>
            env.KV.put(`local:code`, code, { expirationTtl: 60 }),
          );
        }
  ...
  return {
    ...
    issuer: issuer({
      ...
      success: async (ctx, value) =>
        Effect.gen(function* () {
          const email = yield* Schema.decodeUnknown(Email)(value.claims.email);
