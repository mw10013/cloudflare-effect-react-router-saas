...
import { SchemaEx, D1 } from "@workspace/shared"
    ...
    'subscriptionStatus', a.subscriptionStatus,
      ...
      'role', am.role,
        ...
        'deletedAt', u1.deletedAt
          ...
          Effect.flatMap(Schema.decodeUnknown(SchemaEx.DataFromResult(Schema.Array(Customer))))
        ),
...
where a.accountId = ?1 and am.userId = ?2 and am.status = 'active'`
          ...
          d1.first,
          Effect.flatMap(Effect.fromNullable),
          Effect.flatMap(Schema.decodeUnknown(SchemaEx.DataFromResult(AccountWithUser)))
        ),
...
where am.userId = ? and am.status = ?`
          ...
          d1.first,
          Effect.flatMap(Effect.fromNullable),
          Effect.flatMap(Schema.decodeUnknown(SchemaEx.DataFromResult(Schema.Array(AccountMemberWithAccount))))
        ),
...
select json_group_array(json_object(
  ...
  'role', am.role,
    ...
    'deletedAt', u.deletedAt 
          ...
          d1.first,
          Effect.flatMap(Effect.fromNullable),
          Effect.flatMap(Schema.decodeUnknown(SchemaEx.DataFromResult(Schema.Array(AccountMemberWithUser))))
        ),
...
select json_object(
	...
	'active', (
		...
		where reason = 'active'
            ...
            d1.first,
            Effect.flatMap(Effect.fromNullable),
            Effect.flatMap(Schema.decodeUnknown(SchemaEx.DataFromResult(DataSchema)))
          )
        })
    }
  })
