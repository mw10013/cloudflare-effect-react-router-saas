...
export class Ses extends Effect.Service<Ses>()("Ses", {
  ...
  effect: Effect.gen(function* () {
    const EMAIL_WHITE_LIST = yield* Config.withDefault(
      Config.array(Config.string(), "EMAIL_WHITE_LIST"),
      [] as string[],
    );
    const AWS_ACCESS_KEY_ID = yield* Config.redacted(
      Config.nonEmptyString("AWS_ACCESS_KEY_ID"),
    );
    const AWS_SECRET_ACCESS_KEY = yield* Config.redacted(
      Config.nonEmptyString("AWS_SECRET_ACCESS_KEY"),
    );
    ...
    return {
      sendEmail: ({
        ...
        Effect.gen(function* () {
          ...
          if (EMAIL_WHITE_LIST.length > 0 && !EMAIL_WHITE_LIST.includes(to)) {
          ...
          if (!response.ok) {
            return yield* Effect.tryPromise(() => response.text()).pipe(
              Effect.flatMap((text) =>
                Effect.fail(
                  new Error(
                    `Error sending email: ${response.status} ${response.statusText} ${text}`,
                  ),
                ),
              ),
