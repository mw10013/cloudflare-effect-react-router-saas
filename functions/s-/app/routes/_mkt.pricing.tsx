...
import {
  ...
  CardHeader,
  CardTitle,
} from "@workspace/ui/components/ui/card"; // Import Card components
...
export const loader = ReactRouterEx.routeEffect(() =>
  Stripe.getPrices().pipe(Effect.map((prices) => ({ prices }))),
);
...
export const action = ReactRouterEx.routeEffect(
  ({ request, context }: Route.ActionArgs) =>
    Effect.gen(function* () {
      ...
      const priceId = yield* SchemaEx.decodeRequestFormData({
        request,
        schema: FormDataSchema,
      }).pipe(Effect.map((formData) => formData.priceId));
      const { stripeCustomerId, stripeSubscriptionId } =
        yield* Stripe.ensureStripeCustomerId({
        ...
        : Stripe.createCheckoutSession({
            ...
            Effect.flatMap((session) =>
              typeof session.url === "string"
                ? Effect.succeed(redirect(session.url))
                : Effect.fail(new Error("Missing session url")),
            ),
          );
    }),
);
